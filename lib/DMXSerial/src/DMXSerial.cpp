...
void DMXSerialClass::maxChannel(unsigned int channel)
{
...
uint8_t DMXSerialClass::read(unsigned int channel)
{
  // adjust parameter
  if (channel < 1) channel = 1;
  if (channel > DMXSERIAL_MAX) channel = DMXSERIAL_MAX;
...
void DMXSerialClass::write(unsigned int channel, uint8_t value)
{
  // adjust parameters
  if (channel < 1) channel = 1;
  if (channel > DMXSERIAL_MAX) channel = DMXSERIAL_MAX;
...
ISR(USARTn_UDRE_vect)
{
  ...
  if ((unsigned int)_dmxChannel > _dmxMaxChannel) {
